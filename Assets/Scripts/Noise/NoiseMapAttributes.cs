...
{
	[CreateAssetMenu(fileName = "Noise map", menuName = "Voxel World/Noise/Noise map")]
	public class NoiseMapAttributes : NoiseAttributes, INoise
	{
		public enum Type
		{
			Value,
		...
		public float WidthMagnitude = 50f;
		public float HeightMagnitude = 100f;
		public float MinWorldHeight = 0f;

		private delegate float NoiseMethod(Vector3 point, float frequency);
		...
		private static readonly float[] m_gradients1D = { 1f, -1f };
		private static readonly int[] m_hash = {
		...
		private static readonly Vector2[] m_gradients2D = {
			new Vector2( 1f, 0f),
			new Vector2(-1f, 0f),
			new Vector2( 0f, 1f),
			new Vector2( 0f,-1f),
		...
		private static readonly Vector3[] m_gradients3D = {
		...
		//******************************************************
		private static NoiseMethod[] ValueMethods = {
			Value1D,
			Value2D,
			Value3D
		};
		private static NoiseMethod[] PerlinMethods = {
			Perlin1D,
			Perlin2D,
			Perlin3D
		};
		private static NoiseMethod[][] NoiseMethods = {
			ValueMethods,
			PerlinMethods
		};
		//******************************************************
		...
		{
			NoiseMethod method = NoiseMethods[(int)NoiseType][(int)Dimension];
			float sample = (sum(method, new Vector3(x, y, z) / WidthMagnitude, Frequency, Octaves, Lacunarity, Persistence) + 1) / 2;

			if (NoiseType == Type.Value)
				sample *= 0.5f;

			sample *= HeightMagnitude;

			//Generate a bottom noise
			//int minHeight = (int)(Mathf.PerlinNoise(x * Frequency, z * Frequency) * MinWorldHeight) + 1;

			//sample += MinWorldHeight;

			return sample;
			//Return the sample only if is greater than the min height
			//return sample > MinWorldHeight ? sample : minHeight;
		}
		...
		private static float sum(NoiseMethod method, Vector3 point, float frequency, int octaves, float lacunarity, float persistence)
		{
